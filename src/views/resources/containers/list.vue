<template>
  <div>
    <a-card :title="'容器列表'" :bordered="false">
      ...
      <s-table ref="containersRef"
        ...
        <template slot="Id" slot-scope="text, record">
          <router-link :to="{ path: `/resources/containers/${text}`, params: { containerId: text } }">
    ...
    <coco-web-terminal :visible="visibleTerminal"
                       ...
                       :extend-params="terminalExtendParams"
  ...
  export default {
    ...
    methods: {
      loadContainers(params) {
        params['filter'] = { ...this.filter, ...{ all: true } }
        return invokeApi('/container/list', params).then(response => {
      ...
      terminal(containerId) {
        ...
        this.terminalExtendParams = {
          containerId: containerId
        }
        this.$socket.emit('containerTerminal', this.terminalExtendParams)
      },
      // 变更容器状态
