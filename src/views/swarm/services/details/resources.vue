        ...
        this.resources.push(
          {
            ...
            label: '内存资源预留 (MB)',
            value: _.isEmpty(this.data) || _.isEmpty(this.data['Reservations']) ? 0 : convertSize(this.data['Reservations']['MemoryBytes'], false, false, 1024)
          },
          {
            prop: 'memoryLimits',
            tips: '每个调度任务可用的最大内存(设置为0则表示不限制)',
            label: '内存资源限制 (MB)',
            value: _.isEmpty(this.data) || _.isEmpty(this.data['Limits']) ? 0 : convertSize(this.data['Limits']['MemoryBytes'], false, false, 1024)
          },
          {
